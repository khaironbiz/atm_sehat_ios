//
//  ThermometerConnectViewController.m
...
@implementation ThermometerConnectViewController
...
- (void)viewDidLoad {
    ...
    [[ELThermometerBleManager shareManager] connectPeripheral:self.p];
    
    [self setupUIView];
}
-(void)viewWillDisappear:(BOOL)animated{
    [super viewWillDisappear:animated];
    [[ELThermometerBleManager shareManager] disconnectPeripheral];
}
-(void)addLog:(NSString *)log{
    self.textView.text = [NSString stringWithFormat:@"%@\n%@",log,self.textView.text];
}
-(void)buttonAction:(UIButton *)sender{
    if ([ELThermometerBleManager shareManager].state != ELBluetoothStateDidValidationPass) {
    ...
    if (tag == 1) {
        ...
        [[ELThermometerBleManager shareManager] getBluetoothInfoWithELInetGetCmdType:(ELInetGetCmdTypeGetC_V_P_ID)];
    }else if (tag == 3){
        [[ELThermometerBleManager shareManager] getBluetoothInfoWithELInetGetCmdType:(ELInetGetCmdTypeGetName)];
    }else if (tag == 4){
        [[ELThermometerBleManager shareManager] setBluetoothName:@"AILink"];
    }else if (tag == 5){
        [[ELThermometerBleManager shareManager] changeThermometerUnit:(ELDeviceTemperatureUnit_C)];
    }else if (tag == 6){
